...
export const addToCollection = async (req, res) =>{
    ...
    try {
        ...
        await pool.execute(
            `INSERT IGNORE INTO t_game
                (gamId, gamTitle, gamCoverUrl, gamPlatforms, gamDeveloper, gamPublisher, gamGenre, gamMetacritic, gamReleaseDate)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                game.id,
                game.title       ?? null,
        ...
        const [result] = await pool.execute(
            ...
            VALUES (?, ?, ?, ?)`,
            [status, ownPlatforms ?? null, useId, game.id]
        );
...
export const updateCollectionEntry = async (req, res) => {
    ...
    //Verifier si existe un champ à modifier
    if(!status && rating == undefined && comment === undefined && playtime === undefined && ownPlatforms === undefined)
        return res.status(400).json({ error : "Aucun champ à mettre à jour."});

    const validSatatus = ["aquis", "playing", "termine", "wishlist"];
    if(status && !validSatatus.includes(status))
        return res.status(400).json({error : "Status invalide"});
    if (rating !== undefined && (rating<1 || rating>5))
        return res.status(400).json({error : "La note doit être entre 1 et 5"});

    try {
        //Verifier l'utilisateur et sa collection
        const [rows] = await pool.execute(
            "SELECT colId FROM t_collection_game WHERE colId = ? AND colUseId = ?", [colId, useId]);
        if (rows.length === 0)
        return res.status(404).json({ error: "Entrée introuvable dans votre collection." });
        
        await pool.execute(
            `UPDATE t_collection_game SET
                colStatus       = COALESCE(?, colStatus),
                colRating       = COALESCE(?, colRating),
                colComment      = COALESCE(?, colComment),
                colPlaytime     = COALESCE(?, colPlaytime),
                colOwnPlatforms = COALESCE(?, colOwnPlatforms)
            WHERE colId = ? AND colUseId = ?`,
            [
                status       ?? null,
                rating       ?? null,
                comment      ?? null,
